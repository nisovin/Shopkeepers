...
public final class NMSHandler implements NMSCallProvider {
	...
	public boolean openTradeWindow(String name, List<org.bukkit.inventory.ItemStack[]> recipes, Player player) {
		try {
			...
			// hands, but bypasses craftbukkits interact events and therefore removes the spawn egg from the players
			// hands result: we have to prevent openTradeWindow if the shopkeeper entity is being clicking with a spawn
			// egg in hands
